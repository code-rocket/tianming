...
function order_fee($order, $goods, $consignee)
{
    ...
    // 根据配送id 计算配送费用。 如果是99表示商家后续选定快递之后再改运费
    if( $order['shipping_id'] == 99 ){
        $shipping_cod_fee = 0;
    } elseif ($order['shipping_id'] > 0 && $total['real_goods_count'] > 0)
    {
 ...
 * @return  boolean
 */
//wen20160712 addto_cart方法添加两个参数，镜框颜色，镜腿颜色 car_goods_type 购物车商品类型 1普通商品 2DIY 3非太阳镜
function addto_cart($goods_id, $num = 1, $spec = array(), $parent = 0,$jk_colorid=0,$jt_colorid=0,$jp_type,$jp_y_dushu,$jp_y_tongju,$jp_y_sanguang,$jp_y_zhouwei,$jp_z_dushu,$jp_z_sanguang,$jp_z_zhouwei,$jp_y_xiajiaguang,$car_goods_type=1)
{
	
    //wen
    if($jk_colorid!=0 || $jt_colorid!=0){
        $car_goods_type=2;
    }
   ...
   if(!empty($jp_y_dushu))
   {
	...
	'type'=>$jp_type,
	'y_dushu'=>$jp_y_dushu,
	'y_tongju'=>$jp_y_tongju,
	'y_sanguang'=>$jp_y_sanguang,
	'y_zhouwei'=>$jp_y_zhouwei,
	'z_dushu'=>$jp_z_dushu,
	'z_sanguang'=>$jp_z_sanguang,
	'z_zhouwei'=>$jp_z_zhouwei,
	'y_xiajiaguang'=>$jp_y_xiajiaguang
	);
    ...
    $parent = array(
        ...
        //wen20160712添加镜框颜色和镜腿颜色两个字段
        'jk_colorid'   => $jk_colorid,
        'jt_colorid'   => $jt_colorid,
        'car_goods_type'=>$car_goods_type
		//'jp_canshu'=>$jp_canshu
    ...
    if ($num > 0)
    {
        ...
        else //购物车没有此物品，则插入
        {
			...
			$jingpian_parent = array(
				  'tpye'             => $jingpian_config['type'],
				  'y_dushu'          => $jingpian_config['y_dushu'],
				  'y_tongju'         => $jingpian_config['y_tongju'],
				  'y_sanguang'       => $jingpian_config['y_sanguang'],
				  'y_zhouwei'        => $jingpian_config['y_zhouwei'],
				  'z_dushu'          => $jingpian_config['z_dushu'],
				  'z_sanguang'       => $jingpian_config['z_sanguang'],
				  'z_zhouwei'        => $jingpian_config['z_zhouwei'],
				  'y_xiajiaguang'    => $jingpian_config['y_xiajiaguang'],
				  'z_xiajiaguang'    => $jingpian_config['z_xiajiaguang'],
				  'cart_id'           => $rec_id
			...
			$jp_canshu=array(
			  ...
			  if($car_goods_type==3)
			  {
                 $GLOBALS['db']->autoExecute($GLOBALS['ecs']->table('cart_jingpian'), $jp_canshu, 'INSERT');
			  }
		
		//镜片参数插入
		/* $sql_jingpian = "INSERT INTO `ecs_cart_jingpian`(tpye,y_dushu,y_tongju,y_sanguang,y_zhouwei,z_dushu,z_sanguang,z_zhouwei,y_xiajiaguang,z_xiajiaguang,card_id) VALUES ('".$jingpian_config['type']."','".$jingpian_config['y_dushu']."','".$jingpian_config['y_tongju']."','".$jingpian_config['y_sanguang']."','".$jingpian_config['y_zhouwei']."','".$jingpian_config['z_dushu']."','".$jingpian_config['z_sanguang']."','".$jingpian_config['z_zhouwei']."','".$jingpian_config['y_xiajiaguang']."','".$jingpian_config['z_xiajiaguang']."','".$rec_id."')";
          $db->query($sql_jingpian);*/
	...
	/*$sql1 = "DELETE FROM " . $GLOBALS['ecs']->table('cart_jingpian') . " WHERE session_id = '" . SESS_ID . "' ";
...
function clear_cart($type = CART_GENERAL_GOODS)
{
	...
	$sql1 = "DELETE FROM " . $GLOBALS['ecs']->table('cart_jingpian') .
            " WHERE session_id = '" . SESS_ID . "'";
    $GLOBALS['db']->query($sql1);
...
function get_cart_goods()
{
    ...
    while ($row = $GLOBALS['db']->fetchRow($res))
    {
        ...
        //wen
        if($row['jk_colorid']>0){
            $jk_sql="SELECT color_name FROM " . $GLOBALS['ecs']->table('goods_color') . " WHERE col_id=".$row['jk_colorid'];
            $row['jk_color']=$GLOBALS['db']->getCol($jk_sql);
            $row['jk_color']=$row['jk_color'][0];
        }
        if($row['jt_colorid']>0){
            $jt_sql="SELECT color_name FROM " . $GLOBALS['ecs']->table('goods_color') . " WHERE col_id=".$row['jt_colorid'];
            $row['jt_color']=$GLOBALS['db']->getCol($jt_sql);
            $row['jt_color']=$row['jt_color'][0];
        }
		
		if($row['car_goods_type']=="3")
		{
			$jingpian_canshu = $GLOBALS['db']->getRow("SELECT * FROM " . $GLOBALS['ecs']->table('cart_jingpian') . " WHERE `cart_id`='{$row['rec_id']}'");
		}
		
		switch($jingpian_canshu['type'])
		{
			case "1":
			$t_type="近视";
			break;
	    ...
	    $row['type']=$t_type;
		$row['y_dushu']=$jingpian_canshu['y_dushu'];
		$row['y_tongju']=$jingpian_canshu['y_tongju'];
		$row['y_sanguang']=$jingpian_canshu['y_sanguang'];
		$row['y_zhouwei']=$jingpian_canshu['y_zhouwei'];
		$row['z_dushu']=$jingpian_canshu['z_dushu'];
		$row['z_sanguang']=$jingpian_canshu['z_sanguang'];
		$row['z_zhouwei']=$jingpian_canshu['z_zhouwei'];
		$row['y_xiajiaguang']=$jingpian_canshu['y_xiajiaguang'];
		$row['z_xiajiaguang']=$jingpian_canshu['z_xiajiaguang'];
