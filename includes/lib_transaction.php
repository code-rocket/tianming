...
function edit_profile($profile)
{
    ...
    if (!empty($profile['other']))
    {
       ...
       $sql=" update `ecs_users` set user_name='".$profile['user_name']."' where user_id= '$profile[user_id]'";
...
function get_user_orders($user_id, $num = 10, $start = 0)
{
    ...
    $sql = "SELECT order_id, order_sn, order_status,wuliu_sn, shipping_status, pay_status, add_time, " .
