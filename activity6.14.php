...
function get_zuhe()
{
    $sql = "SELECT * FROM " . $GLOBALS['ecs']->table('goods_activity') .
	 ...
	 {
		echo $val['act_id'];
		 $mb[$key]=$GLOBALS['db']->getAll("SELECT * FROM " . $GLOBALS['ecs']->table('package_goods') .
        " where package_id='".$val['act_id']."'");
		
		foreach($mb[$key] as $key1=>$val1)
		{
			$info[]=$GLOBALS['db']->getAll("SELECT * FROM " . $GLOBALS['ecs']->table('goods') .
