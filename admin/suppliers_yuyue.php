...
if ($_REQUEST['act'] == 'list')
{
    ...
    assign_query_info();
    $smarty->display('suppliers_yuyue_list.htm');
}
...
if ($_REQUEST['act']=='reply')
{
    ...
    /* 获取评论详细信息并进行字符处理 */
    $sql = "SELECT * FROM " .$ecs->table('supplires_yuyue'). " WHERE sup_yuyue_id = '$_REQUEST[id]'";
    ...
    $comment_info['datetime'] = local_date($_CFG['time_format'], $comment_info['datetime']);

   
   $sql = "UPDATE ".$ecs->table('supplires_yuyue')." SET ".
               "is_read     = 1".
               " WHERE sup_yuyue_id = '".$_REQUEST['id']."'";
   $db->query($sql);
   

    /* 模板赋值 */
    $smarty->assign('suppliers',          $comment_info); //评论信息
    ...
    /* 页面显示 */
    assign_query_info();
    $smarty->display('suppliers_yuyue_info.htm');
}
/*------------------------------------------------------ */
//-- 处理 回复用户评论
/*------------------------------------------------------ */
...
elseif ($_REQUEST['act'] == 'remove')
{
    ...
    $sql = "DELETE FROM " .$ecs->table('supplires_yuyue'). " WHERE sup_yuyue_id = '$id'";
    $res = $db->query($sql);

    admin_log('', 'remove', 'ads');

    $url = 'suppliers.php?act=list';
...
function get_comment_list()
{
    ...
    $filter['sort_by']      = empty($_REQUEST['sort_by']) ? 'datetime' : trim($_REQUEST['sort_by']);
    $filter['sort_order']   = empty($_REQUEST['sort_order']) ? 'DESC' : trim($_REQUEST['sort_order']);

    $where = 'and 1=1 ';

    $sql = "SELECT count(*) FROM " .$GLOBALS['ecs']->table('supplires_yuyue'). " WHERE suppliers_id = ".$sup_id." $where";
    ...
    /* 获取预约数据 */
    $arr = array();
    $sql  = "SELECT * FROM " .$GLOBALS['ecs']->table('supplires_yuyue'). " WHERE suppliers_id = ".$sup_id." $where " .
            " ORDER BY datetime".
    ...
    {
        $sql ="SELECT suppliers_name FROM " .$GLOBALS['ecs']->table('suppliers'). " WHERE suppliers_id='$row[suppliers_id]'";
