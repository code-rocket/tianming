...
elseif ($_REQUEST['act'] == 'add_color') {
    ...
    $goods_color_list = goods_color_list($goods_id);


    $smarty->assign('goods_id', $goods_id);

    $smarty->assign('goods_color_list', $goods_color_list);

    $smarty->display('goods_color_list.htm');
} elseif ($_REQUEST['act'] == 'goods_color_edit') {
    $col_id = empty($_REQUEST['col_id']) ? 0 : intval($_REQUEST['col_id']);

    $goods_id = empty($_REQUEST['col_id']) ? 0 : intval($_REQUEST['goods_id']);


    $col_info = goods_color_info($col_id);


    $smarty->assign('col_id', $col_id);

    $smarty->assign('col_info', $col_info);

    $smarty->assign('goods_id', $goods_id);

    $smarty->display('goods_color_info.htm');
} //添加颜色
elseif ($_REQUEST['act'] == 'goods_color_add') {
    ...
    $smarty->display('goods_color_info_add.htm');


} //添加颜色
elseif ($_REQUEST['act'] == 'add_goods_color') {

    $goods_id = empty($_REQUEST['goods_id']) ? 0 : intval($_REQUEST['goods_id']);


    $color_img = $image->upload_image($_FILES['color_img']);


    $color_img1 = $image->upload_image($_FILES['color_img1']);


    // 商品缩略图
    if (isset($_FILES['color_img'])) {
        if ($_FILES['color_img']['error'] == 0) {
            if (!$image->check_img_type($_FILES['color_img']['type'])) {
                sys_msg($_LANG['invalid_goods_thumb'], 1, array(), false);
            }
        } elseif ($_FILES['color_img']['error'] == 1) {
            sys_msg(sprintf($_LANG['goods_thumb_too_big'], $php_maxsize), 1, array(), false);
        } elseif ($_FILES['color_img']['error'] == 2) {
    ...
    // 商品缩略图大图
    if (isset($_FILES['color_img1'])) {
        if ($_FILES['color_img1']['error'] == 0) {
            if (!$image->check_img_type($_FILES['color_img1']['type'])) {
                sys_msg($_LANG['invalid_goods_thumb'], 1, array(), false);
            }
        } elseif ($_FILES['color_img1']['error'] == 1) {
            sys_msg(sprintf($_LANG['goods_thumb_too_big'], $php_maxsize), 1, array(), false);
        } elseif ($_FILES['color_img1']['error'] == 2) {
            sys_msg(sprintf($_LANG['goods_thumb_too_big'], $htm_maxsize), 1, array(), false);
        }
    }
    ...
    $sql = "INSERT INTO " . $ecs->table('goods_color') . " (goods_id, color, color_name,img,big_img)" .
        "VALUES ('$goods_id', '$color', '$color_name','$color_img','$color_img1')";
    $out = $db->query($sql);
    $link = array();
    if ($out) {
        $link[] = array('href' => 'goods.php?act=add_color&goods_id=' . $goods_id . '');
        sys_msg('添加颜色成功', 0, $link);
    } else {
        $link[] = array('href' => 'goods.php?act=add_color&goods_id=' . $goods_id . '');
        sys_msg('添加颜色失败', 0, $link);
    }


} //修改颜色
elseif ($_REQUEST['act'] == 'update_goods_color') {
    $col_id = empty($_REQUEST['col_id']) ? 0 : intval($_REQUEST['col_id']);
    ...
    $set .= "set color = '$color',color_name = '$color_name'";
    ...
    if (!empty($color_img1)) {
        $set .= ",big_img = '$color_img1'";
    }

    $sql = "UPDATE " . $ecs->table('goods_color') . " " . $set . " where col_id='$col_id'";
    $out = $db->query($sql);
    $link = array();
    if ($out) {
        $link[] = array('href' => 'goods.php?act=add_color&goods_id=' . $goods_id . '');
        sys_msg('修改颜色成功', 0, $link);
    } else {
        $link[] = array('href' => 'goods.php?act=add_color&goods_id=' . $goods_id . '');
        sys_msg('修改颜色失败', 0, $link);
    }


} //删除颜色
elseif ($_REQUEST['act'] == 'goods_color_delete') {
    ...
    $sql = "DELETE FROM " . $ecs->table('goods_color') .
        " WHERE col_id=" . $col_id . "";
    $out = $db->query($sql);
    $link = array();
    if ($out) {
...
elseif ($_REQUEST['act'] == 'add' || $_REQUEST['act'] == 'edit' || $_REQUEST['act'] == 'copy') {
    ...
    if ($is_add) {
        ...
        $diy_goods_list = get_diy_goods($goods['goods_id']); // DIY配件
		
		$goods_diy_canshu_list = get_goods_diy_canshu($goods['goods_id']); // DIY眼睛参数
		
		/*echo "<pre>";
		print_r($goods_diy_canshu_list);*/
        ...
        if (isset($GLOBALS['shop_id']) && ($GLOBALS['shop_id'] > 10) && !empty($goods['original_img'])) {
            ...
            $goods['goods_pic'] = get_image_path($_REQUEST['goods_id'], $goods['goods_pic']);
        }
    ...
    $smarty->assign('diy_goods_list', $diy_goods_list);
	
	
	$smarty->assign('goods_diy_canshu_list', $goods_diy_canshu_list);
...
elseif ($_REQUEST['act'] == 'insert' || $_REQUEST['act'] == 'update') {
    ...
    if ($_FILES['goods_pic']['error'] == 0) {
        if (!$image->check_img_type($_FILES['goods_pic']['type'])) {
            sys_msg($_LANG['invalid_goods_pic'], 1, array(), false);
        }
    } elseif ($_FILES['goods_pic']['error'] == 1) {
        sys_msg(sprintf($_LANG['goods_ipic_too_big'], $php_maxsize), 1, array(), false);
    } elseif ($_FILES['goods_pic']['error'] == 2) {
        sys_msg(sprintf($_LANG['goods_pic_too_big'], $htm_maxsize), 1, array(), false);
    }
    ...
    if (isset($_FILES['goods_img']['error'])) // php 4.2 版本才支持 error
    {
        ...
        // 商品图片png
        if ($_FILES['goods_pic']['tmp_name'] != 'none') {
            if (!$image->check_img_type($_FILES['goods_pic']['type'])) {
    ...
    if (($_FILES['goods_img']['tmp_name'] != '' && $_FILES['goods_img']['tmp_name'] != 'none') or (($_POST['goods_img_url'] != $_LANG['lab_picture_url'] && $_POST['goods_img_url'] != 'http://') && $is_url_goods_img = 1)) {
        if ($_REQUEST['goods_id'] > 0) {
            /* 删除原来的图片文件 */
            $sql = "SELECT goods_thumb,goods_pic, goods_img, original_img " .
            ...
            if ($row['goods_pic'] != '' && is_file('../' . $row['goods_pic'])) {
                @unlink('../' . $row['goods_pic']);
            }
            ...
            if ($proc_thumb === false) {
                ...
                get_image_path($_REQUEST[goods_id], $row['goods_pic'], false, 'goods', true);
    ...
    move_uploaded_file($_FILES["goods_pic"]["tmp_name"], "../images/upload/" . $goods_pic);
   
   $jingpian_width=$_POST['jingpian_width'];
   
   $jingjia_width=$_POST['jingjia_width'];
	
   $bituo_width=$_POST['bituo_width'];
	 
   $jingpian_height=$_POST['jingpian_height'];
    ...
    if ($is_insert) {
        if ($code == '') {
            $sql = "INSERT INTO " . $ecs->table('goods') . " (goods_name, goods_name_style, goods_sn, " .
                ...
                "promote_start_date, promote_end_date, goods_img,goods_pic, goods_thumb, original_img, keywords, goods_brief, " .
                ...
                "'$promote_start_date', '$promote_end_date', '$goods_img','$goods_pic', '$goods_thumb', '$original_img', " .
        ...
        /* 如果有上传图片，删除原来的商品图 */
        $sql = "SELECT goods_thumb, goods_img,goods_pic, original_img " .
            " FROM " . $ecs->table('goods') .
            " WHERE goods_id = '$_REQUEST[goods_id]'";
        $row = $db->getRow($sql);
        ...
        if ($proc_thumb && $goods_pic && $row['goods_pic'] && !goods_parse_url($row['goods_pic'])) {
            @unlink(ROOT_PATH . $row['goods_pic']);
        }
	 ...
	 if ($is_insert)
	  {
		  ...
		  foreach($jingpian_width as $key=>$val)
		 { 
		   $goods_diy_canshu="insert into " . $ecs->table('goods_diy_canshu') . "(goods_id,jingpian_width,jingjia_width,bituo_width,jingpian_height) value('".$goods_id."','".$jingpian_width[$key]."','".$jingjia_width[$key]."','".$bituo_width[$key]."','".$jingpian_height[$key]."')";
		   $jieguo1=$db->query($goods_diy_canshu);
		 }
	 }
	 else
	 { 
		   $select = "SELECT * FROM " . $ecs->table('goods_diy_canshu') . " WHERE goods_id = '$goods_id'";
		   $jieguo=$db->query($select);
		   if($jieguo)
		   {
			   $delete = "DELETE FROM " . $ecs->table('goods_diy_canshu') . " WHERE goods_id = '$goods_id'";
			   
			   $jieguo1=$db->query($delete);
			   if($jieguo1)
			   {
				     foreach($jingpian_width as $key=>$val)
					 {
					   ...
					   $jieguo2=$db->query($goods_diy_canshu);
					 }
			   }
		   }
	 }
    ...
    $goods_pic = reformat_image_name('goods', $goods_id, $goods_pic, 'goods');
    ...
    if ($goods_pic !== false) {
        $db->query("UPDATE " . $ecs->table('goods') . " SET goods_pic = '$goods_pic' WHERE goods_id='$goods_id'");
    }
...
elseif ($_REQUEST['act'] == 'drop_goods') {
    ...
    $sql = "SELECT goods_id, goods_name, is_delete, is_real, goods_thumb, " .
        "goods_img,goods_pic, original_img " .
        "FROM " . $ecs->table('goods') .
	...
	//眼睛参数表
	$sql = "DELETE FROM " . $ecs->table('goods_diy_canshu') . " WHERE goods_id = '$goods_id'";
	//眼睛参数表
...
elseif ($_REQUEST['act'] == 'add_diy_goods') {
    include_once(ROOT_PATH . 'includes/cls_json.php');
    $json = new JSON;
    ...
    $goods_id = $arguments[0];
    $price = $arguments[1];
    $type = $arguments[2]; //134 镜框 //131 镜腿 //45 镜片

    foreach ($fittings AS $val) {
        $sql = "INSERT INTO " . $ecs->table('goods_diy') . " ( goods_id,diy_goods_id, price, type) " .
            "VALUES ('$goods_id', '$val', '$price', '$type')";
        $db->query($sql, 'SILENT');
    }

    $arr = get_diy_goods($goods_id);
    $opt = array();

    foreach ($arr AS $val) {
        $opt[] = array('value' => $val['diy_goods_id'],
            'text' => $val['goods_name'],
            'data' => '');
    }
...
elseif ($_REQUEST['act'] == 'drop_diy_goods') {
    include_once(ROOT_PATH . 'includes/cls_json.php');
    $json = new JSON;
    ...
    $sql = "DELETE FROM " . $ecs->table('goods_diy') .
        " WHERE goods_id='$goods_id' AND " . db_create_in($fittings, 'diy_goods_id');
