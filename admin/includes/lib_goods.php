...
{
    $sql = "SELECT gd.diy_goods_id, CONCAT(g.goods_name, ' -- [', gd.price, ']') AS goods_name " .
 ...
 * @return  array
 */
function get_goods_diy_canshu($goods_id)
{
    $sql = "SELECT * " .
        "FROM " . $GLOBALS['ecs']->table('goods_diy_canshu') . " " .
        "WHERE goods_id = '$goods_id' ";
    $row = $GLOBALS['db']->getAll($sql);
 ...
 * @return  array
 */
function goods_color_list($goods_id)
{


    $sql = "SELECT * FROM " . $GLOBALS['ecs']->table('goods_color') . "  where goods_id='$goods_id'";
...
function goods_color_info($col_id)
{


    $sql = "SELECT * FROM " . $GLOBALS['ecs']->table('goods_color') . "  where col_id='$col_id'";
