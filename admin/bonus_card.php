...
// todo 待办事项，需要实现：增加字段 is_index_ticket
// ALTER TABLE `ecs_bonus_type` ADD COLUMN `is_index_ticket`  tinyint(1) UNSIGNED NOT NULL DEFAULT 0 COMMENT '是否为：首页优惠券 0-否 ，1-是' ;
// ALTER TABLE `ecs_bonus_type` ADD COLUMN `banner_img`  varchar(200)  NOT NULL DEFAULT '' COMMENT '优惠券广告图片' ;
// ALTER TABLE `ecs_user_bonus` ADD COLUMN `is_index_ticket`  tinyint(1) UNSIGNED NOT NULL DEFAULT 0 COMMENT '是否为：首页优惠券 0-否 ，1-是' ;
...
if ($_REQUEST['act'] == 'list') { 
    ...
    $smarty->assign('action_link', array('text' => $_LANG['bonustype_add'], 'href' => 'bonus_card.php?act=add'));
	...
	if( !empty($list['item']) ){
		foreach ( $list['item'] as & $item ) {
		    $item['min_amount'] = $item['min_goods_amount'];
		}
	}
    ...
    assign_query_info();
    $smarty->display('bonus_card_type.htm');
}
...
if ($_REQUEST['act'] == 'remove') { 
    ...
    $url = 'bonus_card.php?act=query&' . str_replace('act=remove', '', $_SERVER['QUERY_STRING']);
...
if ($_REQUEST['act'] == 'add') { 
    ...
    $smarty->assign('action_link',  array('href' => 'bonus_card.php?act=list', 'text' => $_LANG['04_bonustype_list']));
    ...
    assign_query_info();
    $smarty->display('bonus_card_type_info.htm');
}
...
if ($_REQUEST['act'] == 'insert') { 
    ...
    /* 插入数据库。 */
    $sql = "INSERT INTO ".$ecs->table('bonus_type')." (type_name, type_money,send_start_date,send_end_date,use_start_date,use_end_date,send_type,min_amount,min_goods_amount,is_index_ticket)
    VALUES ('$type_name',
            '$_POST[type_money]',
            '$send_startdate',
            '$send_enddate',
            ...
            '$use_enddate',
            '$_POST[send_type]',
            '$min_amount','" . floatval($_POST['min_goods_amount']) . "', '1' )";
    ...
    $link[0]['href'] = 'bonus_card.php?act=add';

    $link[1]['text'] = $_LANG['back_list'];
    $link[1]['href'] = 'bonus_card.php?act=list';
...
if ($_REQUEST['act'] == 'edit') { 
    ...
    $smarty->assign('action_link', array('href' => 'bonus_card.php?act=list&' . list_link_postfix(), 'text' => $_LANG['04_bonustype_list']));
...
if ($_REQUEST['act'] == 'update') { 
   ...
   /* 提示信息 */
   $link[] = array('text' => $_LANG['back_list'], 'href' => 'bonus_card.php?act=list&' . list_link_postfix());
...
if ($_REQUEST['act'] == 'send_by_user') { 
    ...
    foreach ($user_list AS $key => $val) { 
        ...
        if (add_to_maillist($val['user_name'], $val['email'], $tpl['template_subject'], $content, $tpl['is_html'])) { 
            ...
            $sql = "INSERT INTO " . $ecs->table('user_bonus') .
                    "(bonus_type_id, bonus_sn, user_id, used_time, order_id, emailed, is_index_ticket) " .
                    "VALUES ('$_REQUEST[id]', 0, '$val[user_id]', 0, 0, " .BONUS_MAIL_SUCCEED. ", 1)";
            $db->query($sql);
        } else { 
            /* 邮件发送失败，更新数据库 */
            $sql = "INSERT INTO " . $ecs->table('user_bonus') .
                    "(bonus_type_id, bonus_sn, user_id, used_time, order_id, emailed, is_index_ticket) " .
                    "VALUES ('$_REQUEST[id]', 0, '$val[user_id]', 0, 0, " .BONUS_MAIL_FAIL. ", 1)";
            $db->query($sql);
        }
    ...
    if ($send_count > ($start + $limit)) { 
        /*  */
        $href = "bonus_card.php?act=send_by_user&start=" . ($start+$limit) . "&limit=$limit&id=$_REQUEST[id]&";
    ...
    $link[] = array('text' => $_LANG['back_list'], 'href' => 'bonus_card.php?act=list');
...
if ($_REQUEST['act'] == 'send_mail') { 
    ...
    $link[0]['href'] = 'bonus_card.php?act=bonus_list&bonus_type=' . $bonus['bonus_type_id'];
...
if ($_REQUEST['act'] == 'send_by_print') { 
    ...
    for ($i = 0, $j = 0; $i < $bonus_sum; $i++) { 
        ...
        $db->query("INSERT INTO ".$ecs->table('user_bonus')." (bonus_type_id, bonus_sn, is_index_ticket) VALUES('$bonus_typeid', '$bonus_sn' , 1)");
    ...
    /* 提示信息 */
    $link[0]['text'] = $_LANG['back_bonus_list'];
    $link[0]['href'] = 'bonus_card.php?act=bonus_list&bonus_type=' . $bonus_typeid;
...
elseif ($_REQUEST['act'] == 'remove_bonus') { 
    ...
    $url = 'bonus_card.php?act=query_bonus&' . str_replace('act=remove_bonus', '', $_SERVER['QUERY_STRING']);
    ...
    if (isset($_POST['checkboxes'])) { 
        ...
        if (isset($_POST['drop'])) { 
            ...
            $link[] = array('text' => $_LANG['back_bonus_list'],
                'href' => 'bonus_card.php?act=bonus_list&bonus_type='. $bonus_type_id);
...
function get_type_list() { 
    ...
    $sql = "SELECT bonus_type_id, COUNT(*) AS sent_count".
            ...
            " WHERE is_index_ticket = 1 GROUP BY bonus_type_id";
    ...
    $sql = "SELECT bonus_type_id, COUNT(*) AS used_count".
            " FROM " .$GLOBALS['ecs']->table('user_bonus') .
            " WHERE is_index_ticket = 1 AND used_time > 0".
    ...
    if ($result === false) { 
        ...
        $sql = "SELECT COUNT(*) FROM ".$GLOBALS['ecs']->table('bonus_type').' WHERE is_index_ticket = 1 ';
        $filter['record_count'] = $GLOBALS['db']->getOne($sql);

        /* 分页大小 */
        $filter = page_and_size($filter);

        $sql = "SELECT * FROM " .$GLOBALS['ecs']->table('bonus_type'). " WHERE is_index_ticket = 1  ORDER BY $filter[sort_by] $filter[sort_order]";
...
function get_bonus_list() { 
	...
	$cond =  ' WHERE ub.is_index_ticket = 1';
    $where = empty($filter['bonus_type']) ? $cond : $cond." AND  bonus_type_id='$filter[bonus_type]'";
...
function send_bonus_mail($bonus_type_id, $bonus_id_list) { 
    ...
    $sql = "SELECT b.bonus_id, u.user_name, u.email " .
            ...
            " WHERE b.user_id = u.user_id    AND   b.is_index_ticket = 1  " .
