...
elseif ($_REQUEST['act'] == 'menu')
{
        ...
        if (is_array($val))  {
            ...
            foreach ($val AS $k => $v)  {
                if ( isset($purview[$k])){
					...
					// todo : debug
					// echo '<pre>$purview[$k] ';print_r( $purview[$k] );echo '</pre>'; 
	...
	// 后台菜单由4个文件 决定：
	//  admin/includes/inc_menu.php ( 指定名称和链接 )   admin/includes/inc_priv.php （指定权限名称）    数据库： admin_user.用户账号名.action_list  用户的权限列表 字符串    菜单语言包 文件：languages\zh_cn\admin\common.php

	// todo : debug
    // todo : debug
    //echo '<pre>$_SESSION[action_list]：';print_r( $_SESSION['action_list'] ); echo '</pre> ';
