...
//-- 处理调货
/*------------------------------------------------------ */
if ($_REQUEST['act'] == 'transfer_cargo') {
    $id = intval($_REQUEST['id']);
	
	
	$apply_user = intval($_REQUEST['apply_user']);
	
    /* 获取活动数据 */
    $sql = 'SELECT * FROM ' . $GLOBALS['ecs']->table('suppliers');
    $rows = $GLOBALS['db']->getAll($sql);
    $smarty->assign('suppliers', $rows);
	
	
	
	
	
    $bookinfo = get_booking_info($id);
	
	/*echo "<pre>";
	print_r($apply_user);
*/
    $smarty->assign('booking', $bookinfo);
    $apply_user_sql = 'SELECT * FROM ' . $GLOBALS['ecs']->table('transfer_cargo') . ' where rec_id = ' .$bookinfo["rec_id"].'';
	
	
	
	// print_r($apply_user_sql);die;
	
    $apply_user = $GLOBALS['db']->getRow($apply_user_sql);
	
	//print_r($apply_user);die;
	
    $apply_user_sql1 = 'SELECT * FROM ' . $GLOBALS['ecs']->table('suppliers') . ' where suppliers_id = ' . $apply_user["apply_user"].'';
	
    $apply_user = $GLOBALS['db']->getRow($apply_user_sql1);
...
//        $proportion_a = $_POST["proportion_a"];
//        $proportion_b = $_POST["proportion_b"];
//        /* 插入数据 */
//        $sql = "UPDATE  " . $ecs->table('transfer_cargo') .
//            " SET send_user=$send_user,proportion_a=$proportion_a,proportion_b=$proportion_b,end_time=" . time() . " WHERE rec_id=$rec_id";
//        if ($db->query($sql)) {
//            $sql = "UPDATE  " . $ecs->table('booking_goods') .
//                " SET transfer_cargo_status=2 WHERE rec_id=$rec_id";
//            $db->query($sql);
...
//    }
    ...
    $smarty->assign('apply_user', $apply_user);
    $smarty->assign('ur_here', "调货单");
    $smarty->display('transfer_cargo.htm');
...
//-- 处理调货
/*------------------------------------------------------ */
if ($_REQUEST['act'] == 'see_transfer_cargo') {
   ...
   // print_r($bookinfo);die;
    ...
    $apply_user1 = $GLOBALS['db']->getRow($apply_user_sql);
	
	//print_r($apply_user);die;
	
    $apply_user_sql1 = 'SELECT * FROM ' . $GLOBALS['ecs']->table('suppliers') . ' where suppliers_id = ' . $apply_user1["apply_user"].'';
	
	$apply_user_sql2 = 'SELECT * FROM ' . $GLOBALS['ecs']->table('suppliers') . ' where suppliers_id = ' . $apply_user1["send_user"].'';
	
    $apply_user = $GLOBALS['db']->getRow($apply_user_sql1);
	
	$send_user = $GLOBALS['db']->getRow($apply_user_sql2);


    $smarty->assign('apply_user', $apply_user);
	
	$smarty->assign('send_user', $send_user);
	
	$smarty->assign('apply_info', $apply_user1);
	
    $smarty->assign('ur_here', "调货单详情");
		 
	$smarty->display('transfer_cargo_info.htm');
...
//-- 查看调货详情
/*------------------------------------------------------ */
if ($_REQUEST['act'] == 'transfer_cargo1') {
...
//-- 申请配货
/*------------------------------------------------------ */
if ($_REQUEST['act'] == 'apply_transfer_cargo') {
    $id = intval($_REQUEST['id']);
    /* 获取活动数据 */
	
	
	
    $sql = 'SELECT * FROM ' . $GLOBALS['ecs']->table('suppliers') . ' where user_id =' . $_SESSION["admin_id"];
    $row = $GLOBALS['db']->getRow($sql);
    $smarty->assign('supplier', $row);
    $smarty->assign('booking', get_booking_info($id));
    $smarty->assign('ur_here', "填写申请调货单");
    
    if ($_SERVER['REQUEST_METHOD'] == "POST") {
        $rec_id = $_POST["rec_id"];
		
		$t_infos = $GLOBALS['db']->getRow('SELECT * FROM ' . $GLOBALS['ecs']->table('transfer_cargo') . ' where rec_id = ' . $rec_id.'');
        
		if(empty($t_infos))
		{
				$apply_user = $_POST["apply_user"];
				/* 插入数据 */
				$sql = "INSERT INTO " . $ecs->table('transfer_cargo') . " (apply_user,rec_id,apply_time) VALUES ($apply_user,$rec_id," . time() . ")";
						
				if ($db->query($sql)) {
					$sql = "UPDATE  " . $ecs->table('booking_goods') .
						" SET transfer_cargo_status=1 WHERE rec_id='$_REQUEST[rec_id]'";
					$db->query($sql);
	...
	$smarty->display('apply_transfer_cargo.htm');
...
function get_bookinglist()
{
    ...
    /* 获取活动数据 */
    $sql = 'SELECT bg.rec_id,bg.link_man, g.goods_id, g.goods_name, bg.goods_number, bg.booking_time,bg.is_transfer_cargo,bg.transfer_cargo_status, bg.is_dispose ' .
