...
if($action=="get_all")
{
	...
	$one=$db->GetAll("SELECT * from  " . $ecs->table('delivery_order') . " ");
	foreach($one as $key=>$val)
	{
		$t_time=date('Ymd',$val['add_time']);
		if($time==$t_time)
		{
			$two=$db->GetRow("SELECT * from  " . $ecs->table('order_info') . " where order_id= '".$val['order_id']."' ");
			
			$all_price+=$two['money_paid'];
...
elseif ($_REQUEST['act'] == 'info')
{
    ...
    else
    {
        ...
        $sql = "SELECT * FROM " . $ecs->table('production_process') . " WHERE order_id = '$order_id'";
        $info = $db->getRow($sql);
...
elseif ($_REQUEST['act'] == 'peijing')
{

    $chuyan=$_POST['chuyan'];
    $jiagong=$_POST['jiagong'];
    $zuzhuang=$_POST['zuzhuang'];
    $zhengxing=$_POST['zhengxing'];
    $zhijian=$_POST['zhijian'];
    ...
    if($goods)
    {
        $update=$db->query("UPDATE " . $ecs->table('production_process') . " SET chuyan = '$chuyan',jiagong = '$jiagong',zuzhuang = '$zuzhuang',zhijian = '$zhijian' WHERE order_id = '$order_id'");
    }
    else
    {
        $update=$db->query("INSERT INTO " . $ecs->table('production_process') . " (order_id, chuyan, jiagong, zuzhuang,zhijian)" .
            "VALUES ('$order_id', '$chuyan', '$jiagong', '$zuzhuang','$zhijian')");
    }

    if ($update) {
        $link[] = array('href' => 'order.php?act=info&order_id=' . $order_id . '');
        sys_msg('操作成功', 0, $link);
    } else {
        $link[] = array('href' => 'order.php?act=info&order_id=' . $order_id . '');
        sys_msg('操作失败', 0, $link);
...
elseif ($_REQUEST['act'] == 'wuliu_update')
{
    $wuliu_sn=$_POST['wuliu_sn'];
    $order_id=$_POST['order_id'];

    $update=$db->query("UPDATE " . $ecs->table('order_info') . " SET wuliu_sn = '$wuliu_sn' WHERE order_id = '$order_id'");
    if ($update) {
        $link[] = array('href' => 'order.php?act=info&order_id=' . $order_id . '');
        sys_msg('操作成功', 0, $link);
    } else {
...
elseif ($_REQUEST['act'] == 'add' || $_REQUEST['act'] == 'edit')
{
    ...
    /* 取得参数 step */
    $step_list = array('user','peijingren','wuliu_sn', 'goods', 'consignee', 'shipping', 'payment', 'other', 'money');
    ...
    elseif ('shipping' == $step)
    {
	  ...
	  // 选择配送方式
    elseif ('wuliu_sn' == $step)
    {
        $sql = "SELECT * FROM " . $ecs->table('order_info') . " WHERE order_id = '$_REQUEST[order_id]'";
        $info = $db->getRow($sql);
       ...
       // print_r($info);
        ...
        $smarty->display('wuliu.htm');

    }

    // 选择配送方式
    elseif ('peijingren' == $step)
    {
        $sql = "SELECT * FROM " . $ecs->table('production_process') . " WHERE order_id = '$_REQUEST[order_id]'";
        $info = $db->getRow($sql);
 ...
 * 取得制作信息
 */
function zhizuo_info($order_id)
{
    
	
    $row = $db->getRow("SELECT * from  " . $ecs->table('production_process') . " where order_id='".$order_id."'"); //查询
	if(!empty($row))
	{
      return $row;
	}
