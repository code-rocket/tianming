...
if ($_REQUEST['step'] == 'add_to_cart')
{
	...
	$_POST['diy_config'] = json_str_iconv($_POST['diy_config']);
	
    $_POST['jingpian_config'] = json_str_iconv($_POST['jingpian_config']);
	...
	$diy_config = $json->decode($_POST['diy_config']);
	
    $jingpian_config = $json->decode($_POST['jingpian_config']);
    ...
    {
        // 更新：添加到购物车
        //wen20160712 addto_cart方法添加两个参数，镜框颜色，镜腿颜色
        if (addto_cart($goods->goods_id, $goods->number, $goods->spec, $goods->parent,$diy_config->jingkuang_colorid,$diy_config->jingtui_colorid,$jingpian_config->type,$jingpian_config->y_dushu,$jingpian_config->y_tongju,$jingpian_config->y_sanguang,$jingpian_config->y_zhouwei,$jingpian_config->z_dushu,$jingpian_config->z_sanguang,$jingpian_config->z_zhouwei,$jingpian_config->y_xiajiaguang,1))
        {
...
elseif ($_REQUEST['step'] == 'shipping_change')
{
    ...
    // 如果没有该地区的物流配送, 由商家后来选择1个补上运费
    if( empty($shipping_list) ){
    	$result['content'] = '
            <table width="100%"  cellpadding="0" cellspacing="0" >
            <tr>
                <td width="30%" height=25  valign=top>
                <input name="shipping" type="radio" value="99" checked="true" supportCod="0" insure="0" onclick="selectShipping(this)" />该地区未设定配送，由商家选定快递.
                约[￥15.00]
                </td>
                <td width="70%" valign=top>江、浙、沪地区首重为15元/KG，其他地区18元/KG， 续重均为5-6元/KG， 云南地区为8元</td>
            </tr>
            </table>
    	';
    } else{
...
elseif ($_REQUEST['step'] == 'done')
{
    ...
    $sql = "INSERT INTO " . $ecs->table('order_goods') . "( " .
                "order_id, goods_id,jk_colorid,jt_colorid,car_goods_type, goods_name, goods_sn, product_id, goods_number, market_price, ".
                "goods_price, goods_attr, is_real, extension_code, parent_id, is_gift, goods_attr_id) ".
            " SELECT '$new_order_id', goods_id,jk_colorid,jt_colorid,car_goods_type,goods_name, goods_sn, product_id, goods_number, market_price, ".
                "goods_price, goods_attr, is_real, extension_code, parent_id, is_gift, goods_attr_id".
            " FROM " .$ecs->table('cart') .
	...
	$order_goods_id=mysql_insert_id(); 
	
	$res = $GLOBALS['db']->getOne("select car_goods_type from " . $ecs->table('order_goods') . " where rec_id='".$order_goods_id."'");

	
	if($res=="3")
	{ 
	  
	  $info = $GLOBALS['db']->getRow("select * from " . $ecs->table('cart_jingpian') . " where session_id='".SESS_ID."'");
	  
			
	  $sql1 = "INSERT INTO " . $GLOBALS['ecs']->table('order_goods_jingpian') ." (type,y_dushu,y_tongju,y_sanguang,y_zhouwei,z_dushu,z_sanguang,z_zhouwei,y_xiajiaguang,z_xiajiaguang,order_goods_id) " .
                    "VALUES('".$info['type']."','".$info['y_dushu']."','".$info['y_tongju']."','".$info['y_sanguang']."','".$info['y_zhouwei']."','".$info['z_dushu']."','".$info['z_sanguang']."','".$info['z_zhouwei']."','".$info['y_xiajiaguang']."','".$info['z_xiajiaguang']."','".$order_goods_id."')";
...
elseif ($_REQUEST['step'] == 'clear')
{
	...
	$sql1 = "DELETE FROM " . $ecs->table('cart_jingpian') . " WHERE session_id='" . SESS_ID . "'";
    $db->query($sql1);
...
else
{
  ...
  /*  echo '<pre>';
    ...
    echo '</pre>';*/
    $diy_flag=0;
    foreach ($cart_goods['goods_list'] as $key => &$v) {
        // $fittings_list = get_goods_fittings($parent_list);
        if($v['car_goods_type']==2){
            $v['fittings_list']=get_goods_fittings($v['goods_id']);
            $diy_flag=1;
        }
    ...
    $smarty->assign('diy_flag', $diy_flag);
