  ...
  <div id="" class="user_content clearfix type_login">
    ...
    <div id="" class="user_login Right user_login_new">
      <form  id="JS_login_form" class="username_box" name="formLogin" action="user.php" method="post" onSubmit="return userLogin()">
        ...
        <div class="clearfix user_type_user">
          <h3 class="user_type Left">手机号码</h3>
        </div>
        <p>
          <input value="" type="text" name="extend_field5" class="users_input" id="login_uname" tabindex="1">
        </p>
      ...
      <tr>
                  <td height="30" colspan="4"><p>使用合作网站账号登录滁州家居建材网：</p>
                  <p></p></td>
      </tr>
                <tr>
    ...
    <div id="" class="user_register Left" style="padding-top:20px;color:#555;width:436px;">
      <div class="f14 red c bold" style="width:436px;font-family:微软雅黑">天明眼镜全国<span id="JS_footer_expr_count">多</span>家实体店恭候您的光临！</div>
...
?>
          ...
          <p style="margin-bottom:25px">
            ...
            <input type="text" name="extend_field<?php echo $this->_var['field']['id']; ?>" id="mobile"  class="register_input"  autocomplete="off" tabindex="10">
            <?php if ($this->_var['field']['is_need']): ?>
         ...
         <!--  <div id="username_notice" class="error error2" style=" border:1px solid #FFFFFF; background-color:#FFFFFF;visibility: visible;"> *</div> -->
          ...
          <?php if ($this->_var['enabled_phoneMsg']): ?>


            <p>
              <label class="tips">短信码：</label>
              <input type="text" name="phoneCode" class="register_input verify_input" tabindex="3">
              <span id="bt_send_msg" style=" vertical-align: bottom; display: inline-block; background: #46C972;color:white; padding: 4px 12px; overflow: hidden; color: #FFFFFF; text-decoration: none;line-height: 20px;cursor: pointer;" >发送到手机</span>

            </p>
            <div id="conform_phoneMsg_notice" class="error error2" style=" border:1px solid #FFFFFF; background-color:#FFFFFF;visibility: visible;"> * 请输入短信验证码</div>
            ...
            <script type="text/javascript">
			  ...
			  var msg_timer = 60;
			  var msg_timer_handler ;
              $("#bt_send_msg").click(function(){

				  	// 非延时时间内才可点击
					if( msg_timer != 60  ) return;

					var phone = $("#mobile").val().replace(/[^\d]/g, '');
					if( phone.length != 11 ){ //  || phone.length > 15
						alert('手机号码不正确');
						return;
					}

                    $.get( "user.php?act=reg_sms&phone=" + phone + "&_="+(new Date().getTime()), function(json){

                      json = $.parseJSON(json);

                      if( json.status == 1 ){
                        alert('短信发送成功，请使用您的手机收取验证码。');

                        // 更改为灰色状态, 倒计时延时60秒才能再次点击
                        $("#bt_send_msg").css( "background", "#d8d8d8" );

                        msg_timer_handler = window.setInterval(function(){
                           if( msg_timer <= 0 ){
                             msg_timer = 60;

                             // 恢复原来的样式
                             $("#bt_send_msg").html( "发送到手机" );
                             $("#bt_send_msg").css( "background", "#46C972" );
                             window.clearInterval(msg_timer_handler);

                           } else{

                             $("#bt_send_msg").html("还需等待"+msg_timer+"s");
