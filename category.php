...
if (!$smarty->is_cached('category.dwt', $cache_id)) {
	...
	foreach ( $goodslist as & $v ) {
 		$v['name'] = mb_substr( $v['name'] ,0 ,24, 'utf-8' );
	}

    $smarty->assign('goods_list',       $goodslist);
	$smarty->assign('goods_list1',       $goodslist);

    $smarty->assign('page_count',     range( 1, $max_page ) );
...
function get_goods_color($goods_id) {
	//$goods_id=28;
   $row=$GLOBALS['db']->getAll('SELECT * FROM ' . $GLOBALS['ecs']->table('goods_color') .
...
function category_get_goods($children, $brand, $min, $max, $ext, $size, $page, $sort, $order) {
    ...
    while ($row = $GLOBALS['db']->fetchRow($res)) {
		...
		$arr[$row['goods_id']]['goods_color']       = get_goods_color($row['goods_id']);
		//$arr[$row['goods_id']]['goods_color']       = $row['goods_type'];
