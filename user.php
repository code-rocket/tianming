...
$not_login_arr =
array('login','act_login','register','act_register','act_edit_password','get_password','send_pwd_email','password', 'signin', 'add_tag', 'collect', 'return_to_cart', 'logout', 'email_list', 'validate_email', 'send_hash_mail', 'order_query', 'is_registered', 'check_email','clear_history','qpassword_name', 'get_passwd_question', 'check_answer' , 'oath' , 'oath_login', 'reg_sms');
...
if ($action == 'reg_sms') {
	...
	if( empty($phone) ) exit(json_encode(array('status'=>'-1', 'msg'=>'手机号不能为空',)));

	if( !preg_match("/^0?1((3|8)[0-9]|5[0-35-9]|4[57])\d{8}$/", $phone) ) exit(json_encode(array('status'=>'-1', 'msg'=>'手机号必须是有效的号码',)));
	...
	$msg   = '您在天明眼镜店的注册验证码是：'.$code;
	//$msg   = sprintf( '您有新订单.收货人:%s 电话:%s',$code, $phone );

	// 发送短信
	/*
	require_once(ROOT_PATH . 'includes/cls_sms.php');
	$sms = new sms();
	$res = $sms->send( $phone, $msg);

	define('IN_ECTOUCH',1);
	require_once(ROOT_PATH . 'mobile/include/library/EcsSms.class.php');
	require_once(ROOT_PATH . 'mobile/include/vendor/Http.class.php');
	$sms = new EcsSms();
	$res = $sms->send( $phone, $msg);
	*/


	function ztSms( $mobile, $msg){
		...
		//$msg = "天明眼睛（" . $row2['suppliers_name'] . "分店） 地址：" . $row2['address'] . "   百度地图导航：http://" . $_SERVER["HTTP_HOST"] . "/baidumap.php?x=" . $xy[0] . "&y=" . $xy[1];
		$data = array(
			"username"	=> 'tianming',
			"password"	=> $password,
			"tkey"		=> $tkey,
		...
		require ROOT_PATH . 'sms_api/Util.php' ;
	...
	$_SESSION['reg_mobileCode'] = "{$code},{$expire}";
...
if ($action == 'register')
{
    ...
    /* 手机短信 验证*/
    //if ((intval($_CFG['captcha']) & CAPTCHA_REGISTER) && gd_version() > 0) {
        $smarty->assign('enabled_phoneMsg', 1);
    //}
...
elseif ($action == 'act_register')
{
    ...
    else
    {
        ...
        $username = isset($_POST['extend_field5']) ? trim($_POST['extend_field5']) : '';
        ...
        // 使用短信验证码，而不是图片认证码
		if( !isset($_POST['phoneCode']) || !isset( $_SESSION['reg_mobileCode']) ){
			show_message('短信验证码不正确.');
		}
        $code = trim( strip_tags($_POST['phoneCode']));
        $sess_code = explode(',',$_SESSION['reg_mobileCode']); // "{$code},{$expire}"
 		if( $code !== $sess_code[0] ){
			show_message('短信验证码不正确.');
		} elseif( $sess_code[1] < time() ){
            show_message('短信验证码 已过期，请重新获取.');
        }
...
elseif ($action == 'wuliu') {
    ...
    $wuliu = $db->getOne("SELECT wuliu_sn FROM " .$ecs->table('order_info'). " WHERE order_id = '$order_id'");
    echo "<pre>";
    print_r($orderid);
    echo "</pre>";
    echo $orderid;
    $smarty->assign('num', $wuliu);
    $smarty->display('user_transaction.dwt');
}
/* 查看订单详情 */
