...
/*echo "<pre>";
echo  get_parent_cats(83);*/


//echo "<pre>";
//print_r(get_categories_tree(3));
...
if (!$smarty->is_cached('goods.dwt', $cache_id))
{
    ...
    else
    {
		...
		$goods_color=get_goods_color($goods_id);   //颜色
		
		$suppliers_info=get_attr_suppliers($goods_id);   //体验店
		
		$city_info1=index_get_city1();   //所有城市
		$city_info=index_get_city();   //所有城市
		
		$dushu_info=get_canshu();   //所有眼镜度数
		
		$dushu_info1=get_canshu1();   //所有眼镜度数
		$dushu_info2=get_canshu2();   //所有眼镜度数
		$dushu_info3=get_canshu3();   //所有眼镜度数
		$dushu_info4=get_canshu4();   //所有眼镜度数
		$dushu_info5=get_canshu5();   //所有眼镜度数
		
		$dushu_info6=get_canshu6();   //所有眼镜度数
		...
		$smarty->assign('goods_clors',         $goods_color);                                    //颜色
		
		
		$smarty->assign('city_info1',      $city_info1);           // 所有城市
		
		
		
		$smarty->assign('dushu_info',      $dushu_info);           // 所有度数
		
		$smarty->assign('dushu_info1',      $dushu_info1);           // 所有度数
		$smarty->assign('dushu_info2',      $dushu_info2);           // 所有度数
		$smarty->assign('dushu_info3',      $dushu_info3);           // 所有度数
		$smarty->assign('dushu_info4',      $dushu_info4);           // 所有度数
		$smarty->assign('dushu_info5',      $dushu_info5);           // 所有度数
		$smarty->assign('dushu_info6',      $dushu_info6);           // 所有度数
		
		
		$smarty->assign('or_jingpian',      get_or_jingpian($goods_id));           // 获得是否镜片
		
		
		
		
        $smarty->assign('city_info',       $city_info);           // 所有城市
		
		
		$smarty->assign('suppliers_info_t',  get_suppliers_t_info());           // 体验店信息
		
		//print_r(get_suppliers_t_info());
		
		
		$smarty->assign('suppliers_num',  index_get_suppliers_num());           // 体验店信息
		
		$smarty->assign('suppliers_info',         $suppliers_info);                              //体验店
 ...
 * @return  void
 */
function get_attr_suppliers($goods_id)
{
    $row=$GLOBALS['db']->getOne("SELECT * FROM " . $GLOBALS['ecs']->table('goods') ." WHERE goods_id='$goods_id'");
	$suppliers_id=$row['suppliers_id'];
    $info=$GLOBALS['db']->getOne("SELECT * FROM " . $GLOBALS['ecs']->table('suppliers') ." WHERE suppliers_id='$suppliers_id'");
...
function get_suppliers_t_info()
{
	$mb=$_SESSION['region_id'];
	if(!empty($mb))
	{
		
		$info=$GLOBALS['db']->getAll("SELECT * FROM " . $GLOBALS['ecs']->table('suppliers') ." WHERE city='$mb'");
	}
    else
	{
		
		$tt=$GLOBALS['db']->getRow('SELECT * FROM ' . $GLOBALS['ecs']->table('region') . ' where is_show=1 ORDER BY pin_yin');
		$region_id=$tt['region_id'];
	    $info=$GLOBALS['db']->getAll("SELECT * FROM " . $GLOBALS['ecs']->table('suppliers') ." WHERE city='$region_id'");
 ...
 * @return  void
 */
function get_goods_color($goods_id)
{
    $sql = "SELECT * FROM " . $GLOBALS['ecs']->table('goods_color') .
...
//得到 瞳距
function get_canshu3()
{
	$ary=56;
	round($ary);
	$mr=array();
	for($ary;$ary<=72;$ary+=0.5)
	{
		$mr[]= $ary;
	}
	return $mr;
...
//得到 散光
function get_canshu4()
{
	$ary=-0.25;
	
	$mr=array();
	for($ary;$ary>=-3.75;$ary+=-0.25)
	{
		$mr[]= $ary;
	}
	return $mr;
...
//得到 轴位
function get_canshu5()
{
	$ary=1;
	round($ary);
	$mr=array();
	for($ary;$ary<=180;$ary+=1)
	{
		$mr[]= $ary;
	}
	return $mr;
...
//得到 下加光
function get_canshu6()
{
	$ary=0;
	round($ary);
	$mr=array();
	for($ary;$ary<=4;$ary+=0.25)
	{
		$mr[]= $ary;
	}
	return $mr;
...
//得到是否镜片
function get_or_jingpian($goods_id)
{

	$rows = $GLOBALS['db']->getRow("SELECT * FROM " . $GLOBALS['ecs']->table('goods') . " where goods_id=".$goods_id."");
	  $cat_arr = get_parent_cats($rows['cat_id']);
