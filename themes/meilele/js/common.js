...
function addDiyToCart(goodsId, diy_config, parentId) {
    ...
    $.ajax({
        ...
        cache: false,
        dataType: 'json',     //接受数据格式
        data: {goods:$.toJSON(goods),diy_config:$.toJSON(diy_config)},
        success: addToCartResponse
    });
}
...
function addDiyToCart1(goodsId, jingpian_config, parentId) {
    ...
    $.ajax({
        ...
        cache: false,
        dataType: 'json',     //接受数据格式
        data: {goods:$.toJSON(goods),jingpian_config:$.toJSON(jingpian_config)},
        success: addToCartResponse
    });
}
...
function addToCart2(goodsArr) {
    for (var i = 0; i < goodsArr.length; i++) {
        var parent = (typeof(goodsArr[i].parentId) == "undefined") ? 0 : parseInt(goodsArr[i].parentId);
        addToCart(goodsArr[i].goodsId, parent);
    }
